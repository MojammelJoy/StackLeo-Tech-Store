import { PRODUCT_SLUG_MAX_LENGTH } from "../constants";
...
/**
 * Derives a URL-safe slug from a product name: strips diacritics,
 ...
 * a pure, easily-testable string operation — `service/product.service.ts`
...
export async function generateUniqueSlug(
  ...
  for (;;) {
    ...
    const candidate = `${base.slice(0, PRODUCT_SLUG_MAX_LENGTH - suffix.length)}${suffix}`;
