import { BRAND_SLUG_MAX_LENGTH } from "../constants";

const FALLBACK_SLUG = "brand";
 ...
 * source file. Mirrors `modules/product`/`modules/category`'s
 * `slug.util.ts` exactly. */
...
/**
 * Derives a URL-safe slug from a brand name: strips diacritics,
 ...
 * a pure, easily-testable string operation — `service/brand.service.ts`
...
export async function generateUniqueSlug(
  ...
  for (;;) {
    ...
    const candidate = `${base.slice(0, BRAND_SLUG_MAX_LENGTH - suffix.length)}${suffix}`;
